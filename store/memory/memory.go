...
func updateOne(t *lymbo.Ticket, us lymbo.UpdateSet) {
	...
	if us.Backoff != nil {
		delaySeconds := min(math.Pow(us.Backoff.Base, float64(t.Attempts)), us.Backoff.MaxDelay.Seconds())
		delay := time.Duration(delaySeconds*float64(time.Second)) + us.Backoff.Jitter
...
func (m *Store) PollPending(_ context.Context, req lymbo.PollRequest) (lymbo.PollResult, error) {
	...
	for i := range ready {
		...
		t := ready[i]
		delaySeconds := min(math.Pow(req.BackoffBase, float64(t.Attempts)), req.MaxBackoffDelay.Seconds())
		delay := time.Duration(delaySeconds*float64(time.Second)) + req.TTR
